...
from llamamlx_embeddings.core.models import get_model_info, list_supported_models
...
def quantize_model(
    model_dir: str, output_dir: str, method: str = "int8", overwrite: bool = False
) -> str:
    """
...
def convert_and_quantize(
    ...
    try:
        ...
        logger.info(
            f"Model converted successfully in {results['conversion_time']:.2f} seconds"
        )
        ...
        if quantization:
            ...
            logger.info(
                f"Model quantized successfully in {results['quantization_time']:.2f} seconds"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert and quantize models using llamamlx-embeddings"
    )
    ...
    parser.add_argument(
        "--revision", type=str, default="main", help="Model revision to use"
    )
    parser.add_argument(
        "--dtype",
        type=str,
    ...
    parser.add_argument(
        "--list-models", action="store_true", help="List supported models"
    )
    parser.add_argument(
        "--list-quantization", action="store_true", help="List quantization methods"
    )
    ...
    if not args.model_id:
        ...
        print(
            "Example: python convert_and_quantize.py --model-id BAAI/bge-small-en-v1.5"
        )
    ...
    print(
        f"  Conversion successful: {'✓' if results['conversion_successful'] else '✗'}"
    )
    ...
    if args.quantization:
        print(
            f"  Quantization applied: {'✓' if results['quantization_applied'] else '✗'}"
        )
    ...
    if results["conversion_successful"]:
        model_path = (
            results["quantized_model_dir"]
            if results["quantization_applied"]
            else results["model_dir"]
        )
        print("\nNext steps:")
        print(
            f"  1. Use the model with: python -m llamamlx_embeddings.cli serve --model-id {model_path}"
        )
        print(
            f"  2. Or import in your code: from llamamlx_embeddings import TextEmbedding"
        )
