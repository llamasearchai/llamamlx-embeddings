...
def embed_chunks(chunks: List[str], model: MockEmbedding) -> Dict[str, Any]:
    """
        ...
        "chunks": [
            {
                ...
                "embedding_norm": float(
                    sum(e * e for e in embeddings[i]) ** 0.5
...
def main():
    ...
    for config in chunk_configs:
        ...
        print(
            f"Average chunk length: {sum(len(c) for c in chunks)/len(chunks):.1f} characters"
        )
