...
def create_mock_sparse_embedding(
    text: str, vocab_size: int = 30522, sparsity: float = 0.99
) -> SparseEmbedding:
    ...
    n_elements = max(
        int(vocab_size * (1 - sparsity)), 5
    )  # At least 5 non-zero elements
...
def main():
    ...
    print(
        "In real usage, you would specify a real model like 'prithivida/Splade_PP_en_v1'"
    )
    ...
    sparse_embeddings = [
        create_mock_sparse_embedding(text, vocab_size) for text in texts
    ]
    end_time = time.time()

    # Print sparse embedding information
    print(
        f"Generated {len(sparse_embeddings)} sparse embeddings in {end_time - start_time:.4f} seconds"
    )
    ...
    for i in range(len(sparse_embeddings)):
        for j in range(i + 1, len(sparse_embeddings)):
            ...
            similarity = sparse_dot_product(sparse_embeddings[i], sparse_embeddings[j])
