...
def generate_random_text(
    n_samples: int, mean_len: int = 100, std_len: int = 20
) -> List[str]:
    """
...
def benchmark_model(
    ...
    for batch_size in batch_sizes:
        ...
        if batch_size > n_samples:
            logger.warning(
                f"Skipping batch size {batch_size} as it exceeds number of samples"
            )
            continue
        ...
        batches = [
            texts[i * batch_size : (i + 1) * batch_size] for i in range(num_batches)
        ]
