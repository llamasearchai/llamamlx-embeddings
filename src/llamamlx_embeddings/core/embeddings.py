...
class BaseEmbedding(ABC):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if model_name != "mock" and not hasattr(
            self, "model_info"
        ):  # Skip for mock models
        ...
        for batch in iter_batch(inputs, batch_size):
            ...
            for text in batch:
                if text.startswith("query:") or text.startswith("passage:"):
                    ...
                    "BAAI/bge"
                ) or self.model_name.startswith("intfloat/e5"):
                    #  This is where the logic from open-text-embeddings is crucial
                    if kwargs.get(
                        "is_passage", True
                    ):  # Default to passage if not specified
            ...
            try:
                ...
                for vec in sparse_vecs:
                    ...
                    threshold = kwargs.get(
                        "threshold", 0.0
                    )  # Threshold for sparsity (0 keeps all)
...
class MLXEmbeddingModel:
    ...
    def __init__(
        ...
        self.has_cls_token = "[CLS]" in self.tokenizer.special_tokens_map.get(
            "cls_token", ""
        )
    ...
    def _encode_single(self, text: str) -> np.ndarray:
        ...
        if self.pooling_strategy == "cls" and self.has_cls_token:
            ...
            emb = mx.sum(
                outputs.last_hidden_state * attention_mask.reshape(-1, 1), axis=1
            )
    ...
    def _load_model(self) -> EmbeddingModel:
        ...
        try:
            ...
            if self.embedding_size is None:
                ...
                if hasattr(mlx_model, "config") and hasattr(
                    mlx_model.config, "hidden_size"
                ):
