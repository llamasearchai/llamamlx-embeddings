...
class SparseEmbedding:
    ...
    @classmethod
    def from_dense(
        cls, dense_vector: mx.array, threshold: float = 1e-6
    ) -> "SparseEmbedding":
        """
...
def sparse_dense_mm(
    sparse_matrix: List[SparseEmbedding], dense_matrix: mx.array
) -> mx.array:
    """
...
def sparse_dot_product(
    sparse_emb1: SparseEmbedding, sparse_emb2: SparseEmbedding
) -> float:
    """
...
def convert_to_sparse_embedding(
    dense_vector: Union[mx.array, np.ndarray], threshold: float = 1e-6
) -> SparseEmbedding:
    """
