...
from ..utils.error_handling import require_dependency, safe_import
...
def mean_pooling(
    ...
    if attention_mask is not None:
        ...
        return np.sum(input_array, axis=1) / np.sum(
            attention_mask, axis=1, keepdims=True
        )
    else:
        # Simple mean pooling
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    raise ValueError(
                        f"Could not find ONNX model for {self.model_name}. "
                        f"Error: {str(e)}"
                    )
            ...
            logger.info(
                f"ONNX model loaded with inputs: {input_names}, outputs: {output_names}"
            )
                ...
                if len(outputs) > 0:
                    ...
                    if len(embeddings.shape) == 3:  # [batch_size, seq_len, hidden_dim]
                        embeddings = mean_pooling(
                            embeddings, inputs_encoded.get("attention_mask")
                        )
